...
def ensure_schema():
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            """)
            # snapshot (ya la creaste por UI, pero dejamos IF NOT EXISTS por seguridad)
            cur.execute("""
...
def sync(request: Request):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            # 3) calcular KPIs y guardar snapshot (misma lógica que venías usando)
            ...
            SELECT
              ...
              (SELECT COUNT(*) FROM order_events WHERE status = ANY(%(env)s) AND event_ts >= %(today_start)s) AS despachados_hoy,
            ...
            kpi = cur.fetchone()  # tuple en el orden del SELECT
